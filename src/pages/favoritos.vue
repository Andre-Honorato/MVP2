<template>
  <v-layout full-height>
    <v-container
      ...
      class="pa-6 d-flex flex-column justify-center align-center h-100"
    >
      <v-icon
        color="low"
        size="96"
        >mdi-heart</v-icon
      >
      <h3 class="text-low">Nenhum item favoritado</h3>
    </v-container>
    <v-container
      v-else
      class="pa-6"
    >
      ...
      >
        ...
        >
          <v-btn
            @click.stop="favoriteItem(product)"
            icon
            variant="flat"
            color="transparent"
            width="auto"
...
function updateFavorites() {
  if (storeUser.user.type === 'ong') {
    favorites.value = storeProducts.products.markets.filter(
      (obj) => obj.favorite
    )
  } else {
    isOng.value = true
...
function favoriteItem(product) {
  if (storeUser.user.type === 'ong') {
    ...
    storeProducts.products.markets[index].favorite =
      !storeProducts.products.markets[index].favorite
    updateFavorites()
  } else {
    ...
    storeProducts.products.ongs[index].favorite =
      !storeProducts.products.ongs[index].favorite
    updateFavorites()
  }
}
